...
def main():
    ...
    html_lines = [
        ...
        "      padding:12px 18px;text-decoration:none;border-radius:10px;min-width:220px;",
        ...
        "      </div>",
        "      <div class='controls'>",
        "        <!-- kept minimal: optional search or controls can be added later -->",
        ...
        "",
        "    <div class='grid'>",
        "      <!-- Buttons for each folder will be inserted here by your existing logic -->",
        "      <!-- keep using: <a class='btn' href='FOLDERNAME_report.html'>FOLDERNAME</a> -->",
        "    </div>",
        "",
        "    <footer>",
        ...
        "",
        "  <script>",
        "    // Small helper so that when your code writes <a class='btn' ...> elements into the .grid,",
        "    // each button can include data-name / data-meta attributes for the search to work.",
        "    // This does not alter your existing logic — it's just a compatibility shim.",
        "    document.addEventListener('DOMContentLoaded', ()=>{",
        "      document.querySelectorAll('.grid a.btn').forEach(a=>{",
        "        // normalize for search: prefer dataset attrs if present, otherwise use link text",
